...
class HomepagePresenter extends Nette\Application\UI\Presenter {
    ...
    public function renderGenerateNewTokenForFbId($fbId, $accessToken = null) {
        ...
        try {
            $response = $this->facebookHandler->get('/' . $fbId . '?metadata=1&fields=id,name,picture,metadata{type}', $accessToken);
        } catch (\Exception $e) {
            //echo 'Facebook SDK returned an error: ' . $e->getMessage();
            $this->flashMessage("There have been some error during loading information about selected resource. Please log in again.", "danger");
            $this->actionLogout(false);
        }

        $this->template->selectedResourceInfo = $response->getGraphNode();
